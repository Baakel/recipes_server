...
use crate::models::{UserId, User, UsedIdError, ChosenDeleted, ChosenTimeError, GraphPool};
...
impl<'a, 'r> FromRequest<'a, 'r> for UserId {
    ...
    fn from_request(request: &'a Request<'r>) -> Outcome<Self, Self::Error> {
        ...
        let result = request.local_cache(|| {
            rt.block_on(async {
                get_user_from_db(graph.clone(), &cookie_id).await
            })
        });
        if result.is_none() {
...
impl<'a, 'r> FromRequest<'a, 'r> for User {
    ...
    fn from_request(request: &'a Request<'r>) -> Outcome<Self, ()> {
        ...
        let result = request.local_cache(|| {
            rt.block_on(async {
                get_user_from_db(graph.clone(), &uuid).await
            })
        });
        if result.is_none() {
...
// Use this guard to delete the recipes after 8 days when checking the dashboard. This
...
impl<'a, 'r> FromRequest <'a, 'r> for ChosenDeleted {
    ...
    fn from_request(request: &'a Request<'r>) -> Outcome<Self, Self::Error> {
        ...
        let date_created = request.local_cache(|| {
            rt.block_on(async {
                let mut res = graph
                    .execute(
                        query(
                            "MATCH (u:User)-[c:CHOSEN]-() WHERE u.id = $id RETURN c"
                        ).param("id", uuid.clone()),
                    )
                    .await
                    .expect("Couldn't find that Uuid");

                let row = res.next().await.expect("Couldn't fetch row").expect("Empty row");
        ...
        let eight_days_offset = date_created.checked_sub_signed(Duration::seconds(8*24*60*60)).unwrap();
        if date_created < &eight_days_offset {
            rt.block_on(async {
                graph.run(
