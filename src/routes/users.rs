...
pub fn new_user(user: Json<User>, graph: State<Graph>, rt: State<Runtime>) -> Flash<Redirect> {
    ...
    if !(validate_email(email)) || !(validate_length(username, Some(3), None, None)) ||
    ...
    Flash::success(Redirect::to(uri!("/users", query_users)), "User added to the db.")
}

// TODO: Retrieving a cookie
#[post("/login", format="application/json", data="<user>")]
pub fn login(user: Json<User>, graph: State<Graph>, rt: State<Runtime>, mut cookies: Cookies) ->
String {
    let username = &user.username;
    let password = &user.password.as_bytes();
    let argon2 = Argon2::default();
    ...
    let res_tuple: (String, String) = rt.block_on(async {
        ...
        ).await.expect("Couldn't find that user");

        let row = result.next().await.expect("Couldn't fetch row").expect("Empty row");
        let node: Node = row.get("u").unwrap();
        (node.get("password").expect("No password found"), node.get("id").expect("No id found"))
    });
...
pub fn query_users(rt: State<Runtime>, graph: State<Graph>, flash: Option<FlashMessage>) -> String {
    ...
    format!("Flash was {}\n This is the user vector {:?}", flash.unwrap().msg(), res)
}

#[get("/<name>")]
pub fn get_user(rt: State<Runtime>, graph: State<Graph>, name: String, key: UserId) ->
Option<String> {
